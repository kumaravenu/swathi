...
public class Readingdata1 extends Base {
	...
	public void withinUSA(HashMap<String, String> H) throws IOException {
		...
		try {
			...
			po.fromcitydata(H.get("From"));
			po.tocitydata(H.get("To"));
			po.enterbutton();
			String c = po.gettitle1();
		} catch (Exception e) {
			TakesScreenshot scrShot = ((TakesScreenshot) driver);
			File SrcFile = scrShot.getScreenshotAs(OutputType.FILE);
			File DestFile = new File("C://Vikash/H.get('From')+_H.get('To').png");
	...
	public static Object[][] readdata(Method M) throws IOException {
		FileInputStream f;
		XSSFWorkbook w;
		XSSFSheet s;
		...
		String runmode = run.get_Testcase_Run(testcasename);

		System.out.println("Hiiiii " + testcasename);
		System.out.println("Hi " + runmode);

		f = new FileInputStream(fileName);

		w = new XSSFWorkbook(f);
		s = w.getSheet(testcasename);
